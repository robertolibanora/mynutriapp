...
APP_DIR="/var/www/staging-mynutriapp"
PID_FILE="$APP_DIR/logs/gunicorn.pid"

cd "$APP_DIR"
mkdir -p logs static/uploads storage/audio

BIND="$(grep -E '^GUNICORN_BIND=' "$APP_DIR/.env" 2>/dev/null | cut -d= -f2- | tr -d ' \"')"
BIND="${BIND:-127.0.0.1:8199}"
PORT="${BIND##*:}"
...
if [[ -f "$PID_FILE" ]]; then
  ...
  if [[ -n "$OLD_PID" ]] && kill -0 "$OLD_PID" 2>/dev/null; then
    if health_ok; then
      echo "Staging MyNutriApp gia' in esecuzione su ${BIND} (PID ${OLD_PID})"
      exit 0
    fi
