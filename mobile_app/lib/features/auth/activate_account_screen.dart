...
class _ActivateAccountScreenState extends State<ActivateAccountScreen> {
  ...
  String? _validate() {
    ...
    if (widget.token.trim().isEmpty) {
      return 'Link non valido. Richiedi un nuovo invito al nutrizionista.';
    }
    if (p.length < 8) {
  ...
  Future<void> _submit() async {
    ...
    try {
      final msg = await AppScope.of(context).auth.activateAccount(
            token: widget.token,
      ...
      if (code == 'invalid_token') {
        setState(
          () => _error =
              'Link non valido, scaduto o già usato. Chiedi al nutrizionista di reinviare l\'invito.',
        );
      } else if (code == 'weak_password' || code == 'password_mismatch') {
        setState(() => _error = message ?? 'Password non valida.');
      } else {
        setState(() => _error = message ?? 'Attivazione non riuscita. Riprova.');
      }
    } catch (_) {
      if (!mounted) return;
      ...
      body: SafeArea(
        child: ListView(
          ...
          children: [
            ...
            const Text(
              'Imposta la password per accedere all\'area paziente.',
              textAlign: TextAlign.center,
            ...
            if (missingToken)
              ...
              FilledButton(
                ...
                child: Text(_busy ? 'Attivazione…' : 'Attiva account'),
              ),
            ],
          ],
        ),
