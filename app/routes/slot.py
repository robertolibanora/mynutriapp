...
def orari_settimanali():
    ...
    if request.method == 'POST':
        ...
        try:
            if action == 'aggiungi':
                ...
                uid = require_tenant()
                AgendaService.aggiungi_orario(giorno, ora, note=note, utente_id=uid)
                flash("Orario aggiunto", "success")
            elif action == 'elimina':
                AgendaService.rimuovi_orario(int(request.form['orario_id']), utente_id=require_tenant())
    ...
    orari_per_giorno = AgendaService.get_orari_per_giorno(utente_id=require_tenant())
...
def eccezione_agenda():
    ...
    if request.method == 'POST':
        try:
            ...
            note = (request.form.get('note') or '').strip() or None
            AgendaService.aggiungi_eccezione(
                data_inizio, data_fine, note=note, utente_id=require_tenant()
            )
    ...
    eccezioni = AgendaService.get_eccezioni(future_only=True, utente_id=require_tenant())
...
def elimina_eccezione(eccezione_id):
    try:
        AgendaService.rimuovi_eccezione(eccezione_id, utente_id=require_tenant())
